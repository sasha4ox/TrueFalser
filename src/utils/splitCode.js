...
function splitCode(string) {
  ...
  const findStringWithStartMarker = _filter(splitedStringByEnter, (item) => item.includes('*START*'));
  ...
  if (!isSoloStart) {
    ...
    stringAfterStart = Array.from(separatedStringByEnterAfterStart);
  } else {
    beforeStart = _map(
  ...
  const findStringWithFinishMarker = _filter(splitedStringByEnter, (item) => item.includes('*FINISH*'));
